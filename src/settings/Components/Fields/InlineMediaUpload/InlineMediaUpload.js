...
import "./style.scss";
const InlineMediaUpload = ({ value, onChange = () => { }, preview = true, url = true, library= 'image', placeholder, button_title = "Upload", }) => {
  ...
  return (
    <div className="bPl-inlineMedia-upload-wrapper">
      {value?.url && preview && <div className="bPl-inlineMedia-upload-preview">
        {library === "image" && <img src={value?.url} alt="" />}
        {library === "video" && videoIcon}
        {library === "audio" && audioIcon}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "5px", width: "100%" }}>
        {
          url && <div style={{ width: "100%" }}>
            <BTextControl placeholder={placeholder} style={{ borderRadius: "3px" }} value={value} onChange={val => setData(val)} />
          </div>
        }
        <div>

          <MediaUpload
            allowedTypes={[library]}
            gallery={false}
            multiple={false}
            addToGallery={false}
        ...
        {!preview && value?.url && <Button variant="warning" onClick={() => setData("")}>Remove</Button>}
      </div>
    </div>
  );
};
