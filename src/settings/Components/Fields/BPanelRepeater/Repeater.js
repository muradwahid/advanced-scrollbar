...
const ChevronDownIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" stroke="currentColor" fill="currentColor" height="1em" width="1em" viewBox="0 0 448 512">
    <path d="M201.4 374.6c12.5 12.5 32.8 12.5 45.3 0l160-160c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L224 306.7 86.6 169.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l160 160z" />
  </svg>
);

const ChevronRightIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" stroke="currentColor" fill="currentColor" height="1em" width="1em" viewBox="0 0 320 512">
    <path d="M278.6 233.4c12.5 12.5 12.5 32.8 0 45.3l-160 160c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L210.7 256 73.4 118.6c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l160 160z" />
  </svg>
);

const DragIcon = () => (
  <svg style={{ cursor: "all-scroll" }} stroke="currentColor" fill="currentColor" strokeWidth="0" version="1.2" baseProfile="tiny" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
...
const DeleteIcon = () => (
  <svg stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 384 512" xmlns="http://www.w3.org/2000/svg">
...
const Repeater = ({
  ...
  useEffect(() => {
    setWidth(iconGroupRef.current?.clientWidth || 35);
  }, []);

  const HeaderTitle = () => (
    <div onClick={() => setOpen(!open)} className="panel-repeater-header-title">
      <span>{open ? <ChevronDownIcon /> : <ChevronRightIcon />}</span>
      {title ? <p style={{ margin: "0px" }}>{title}</p> : <p className="repeater-title-placeholder" />}
    </div>
  );
  ...
  const IconGroup = () => (
    <div ref={iconGroupRef} className="panel-repeater-icon-group">
      ...
      {type === "group" && (
        <div className="panel-repeater-icon">
          <DragIcon />
        </div>
      )}
      {copy && (
        <div onClick={handleCopy ? () => handleCopy(index) : null} className="panel-repeater-icon">
          <CopyIcon />
        </div>
      )}
      {del && (
        <div onClick={handleDelete ? () => handleDelete(index) : null} className="panel-repeater-icon">
          <DeleteIcon />
        </div>
      )}
    </div>
      ...
      <div className="panel-repeater-wrapper">
        {type === "group" ? (
          ...
          <div className="panel-without-group-wrapper">
            <div style={{ width: `calc(100% - ${width}px)` }} className="panelChildrenWrapper">
              {children}
            </div>
            <IconGroup />
          </div>
