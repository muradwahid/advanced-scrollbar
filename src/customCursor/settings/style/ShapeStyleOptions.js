...
const ShapeStyleOptions = ({ value, onChange }) => {
  ...
  return (
    <>
      ...
      {
        value?.type === 'glow' && <>
          <RangeControl label="Cursor Size" value={parseInt(value?.glow?.size) >= 0 ? parseInt(value?.glow?.size) : 50} onChange={val => onChange(updateData(value, val, 'glow', 'size'))} min={0} max={200} step={1} />
...
export const GlitchCursorOptions = ({ value = {}, onChange, size, primaryColor, secondaryColor, tertiaryColor }) => {
