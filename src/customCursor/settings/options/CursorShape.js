...
const CursorShape = (props) => {
  const { value = {}, onChange, isPremium, setIsProModalOpen } = props;

  return <>
    <SelectControlPro className='mt10' label={__('Cursor Shape', 'advanced-scrollbar')} labelPosition='edge' value={value?.type || ""} onChange={(val) => onChange(updateData(value, val, 'type'))} options={cursorOptions} isPremium={isPremium} proValues={cursorProOptions} setIsProModalOpen={setIsProModalOpen} />
    {value?.type === "follow" && <>
