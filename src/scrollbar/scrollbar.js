...
const Scrollbar = ({ scrollbarData }) => {
    ...
    const { scrollPercentage, dynamicHeight, isWheel, isLoading, thumbHeight, thumbPosition } = useScrollbar(railRef, thumbRef,showScrollbar, Number(asb_mousescrollstep), Number(asb_scrollspeed), touchBehavior);
    const { cursorPosition } = useMovePosition(asb_floating_scrollbar)
    useEffect(() => {
